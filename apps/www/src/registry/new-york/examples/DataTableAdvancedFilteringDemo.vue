  ...
  <div class="w-full space-y-4">
    ...
    <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
      ...
      <div class="flex items-center gap-2">
              ...
              <div class="space-y-2">
                ...
                <div class="max-h-48 overflow-auto">
                  <div v-for="role in uniqueRoles" :key="role" class="flex items-center space-x-2">
                    <Checkbox
                      :id="`role-${role}`"
                      :model-value="table.getColumn('role')?.getFilterValue()?.includes(role) || false"
                      @update:model-value="(checked) => {
                        const currentValue = table.getColumn('role')?.getFilterValue() || []
        ...
        <Popover>
          <PopoverTrigger as-child>
            <Button variant="outline" size="sm" class="border-dashed">
              ...
              Department
              <Badge
                v-if="table.getColumn('department')?.getFilterValue()?.length"
                variant="secondary"
                class="ml-2 rounded-sm px-1 font-normal"
              >
                {{ table.getColumn('department')?.getFilterValue()?.length }}
              </Badge>
            </Button>
          </PopoverTrigger>
          <PopoverContent class="w-56 p-0" align="start">
              ...
              <div class="space-y-2">
                ...
                <div class="max-h-48 overflow-auto">
                  <div v-for="dept in uniqueDepartments" :key="dept" class="flex items-center space-x-2">
                    <Checkbox
                      :id="`dept-${dept}`"
                      :model-value="table.getColumn('department')?.getFilterValue()?.includes(dept) || false"
                      @update:model-value="(checked) => {
                        const currentValue = table.getColumn('department')?.getFilterValue() || []
                        const newValue = checked
                          ? [...currentValue, dept]
        ...
        <!-- Status Filter -->
        <Select
          :model-value="table.getColumn('status')?.getFilterValue()?.[0] || ''"
