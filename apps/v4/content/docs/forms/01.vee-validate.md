...
```vue showLineNumbers title="Form.vue" {3,10,15}
<template>
...
```vue showLineNumbers title="Form.vue" {3,7,17}
<template>
...
```vue showLineNumbers title="Form.vue" {1,3,8-14,26}
<template>
...
```vue showLineNumbers title="Form.vue" {1,3,8-11,15,23,25}
<template>
...
- For switches, use `:checked="field.value"` and `@update:checked="field.onChange"` for proper binding.
...
```vue showLineNumbers title="Form.vue" {1,3,11,16-19}
<template>
...
```vue showLineNumbers title="Form.vue" {3-5}
import { FieldArray } from 'vee-validate'

<FieldArray v-slot="{ fields, push, remove }" name="emails">
  <!-- Array items go here -->
</FieldArray>
```
...
Map over the `fields` array and create fields for each item. **Make sure to use `field.key` as the key**. For array fields, you need to manually manage the field binding since the FieldArray provides the field values directly.

```vue showLineNumbers title="Form.vue"
<VeeField v-slot="{ errors: fieldErrors }" :name="`emails[${index}].address`">
  <Field
    :key="field.key"
    orientation="horizontal"
  ...
  >
    <FieldContent class="flex-1">
      ...
      <FieldError v-if="fieldErrors.length">
        {{ fieldErrors[0] }}
      </FieldError>
    </FieldContent>
  </Field>
