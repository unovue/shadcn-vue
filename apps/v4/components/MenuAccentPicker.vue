<script setup lang="ts">
import type { MenuAccentValue } from '@/registry/config'
import { MENU_ACCENTS } from '@/registry/config'

defineProps<{
  isMobile: boolean
  anchorRef: HTMLDivElement | null
}>()

const params = useDesignSystemSearchParams()
const currentAccent = computed(() => MENU_ACCENTS.find(m => m.value === params.menuAccent.value) ?? MENU_ACCENTS[0])
</script>

<template>
  <div class="group/picker relative pr-3 md:pr-0">
    <Picker>
      <PickerTrigger>
        <div class="flex flex-col justify-start text-left">
          <div class="text-muted-foreground text-xs">
            Menu Accent
          </div>
          <div class="text-foreground text-sm font-medium">
            {{ currentAccent?.label }}
          </div>
        </div>
        <div class="text-foreground pointer-events-none absolute top-1/2 right-4 flex size-4 -translate-y-1/2 items-center justify-center text-base select-none md:right-2.5">
          <svg
            xmlns="http://www.w3.org/2000/svg"
            width="128"
            height="128"
            viewBox="0 0 24 24"
            fill="none"
            class="size-4 text-foreground"
          >
            <path
              d="M19 12.1294L12.9388 18.207C11.1557 19.9949 10.2641 20.8889 9.16993 20.9877C8.98904 21.0041 8.80705 21.0041 8.62616 20.9877C7.53195 20.8889 6.64039 19.9949 4.85726 18.207L2.83687 16.1811C1.72104 15.0622 1.72104 13.2482 2.83687 12.1294M19 12.1294L10.9184 4.02587M19 12.1294H2.83687M10.9184 4.02587L2.83687 12.1294M10.9184 4.02587L8.89805 2"
              stroke="currentColor"
              stroke-width="2"
              stroke-linecap="round"
              stroke-linejoin="round"
              :class="currentAccent?.value === 'bold' ? 'fill-foreground' : 'fill-muted-foreground/30'"
            />
            <path
              d="M22 20C22 21.1046 21.1046 22 20 22C18.8954 22 18 21.1046 18 20C18 18.8954 20 17 20 17C20 17 22 18.8954 22 20Z"
              stroke="currentColor"
              stroke-width="2"
              stroke-linecap="round"
              stroke-linejoin="round"
              :class="currentAccent?.value === 'bold' ? 'fill-foreground' : 'fill-muted-foreground/30'"
            />
          </svg>
        </div>
      </PickerTrigger>
      <PickerContent
        :anchor="isMobile ? anchorRef : undefined"
        :side="isMobile ? 'top' : 'right'"
        :align="isMobile ? 'center' : 'start'"
      >
        <PickerRadioGroup
          :model-value="currentAccent?.value"
          @update:model-value="(value) => {
            params.menuAccent.value = value as MenuAccentValue
          }"
        >
          <PickerGroup>
            <PickerRadioItem
              v-for="accent in MENU_ACCENTS"
              :key="accent.value"
              :value="accent.value"
              :close-on-click="isMobile"
              :disabled="accent.value === 'bold' && (params.menuColor.value === 'default-translucent' || params.menuColor.value === 'inverted-translucent')"
            >
              {{ accent.label }}
            </PickerRadioItem>
          </PickerGroup>
        </PickerRadioGroup>
      </PickerContent>
    </Picker>
    <LockButton
      param="menuAccent"
      class="absolute top-1/2 right-8 -translate-y-1/2"
    />
  </div>
</template>
